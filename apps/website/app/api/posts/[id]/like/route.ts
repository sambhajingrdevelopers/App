...
export async function POST(
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/v1/posts/${encodeURIComponent(params.id)}/like`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ liked: body.liked }),
      cache: 'no-store'
    });
    ...
    if (!response.ok || !data.success) {
      return NextResponse.json(
        { success: false, message: data?.message || 'Like update failed' },
