...
export async function POST(
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/v1/admin/ads/${params.id}/status`, {
      method: 'POST',
    ...
    if (!response.ok || !data.success) {
      return NextResponse.json(
        { success: false, message: data.message || 'Ad update failed' },
        { status: 500 }
      );
    }

    return NextResponse.json({ success: true, ad: data.ad });
  } catch (error: any) {
    return NextResponse.json(
